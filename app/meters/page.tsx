// app/meters/page.tsx - หน้ามิเตอร์น้ำและไฟฟ้า (public/admin)
...
async function getMeterReadings(cycleId?: number, roomId?: number) {
  try {
    ...
    if (!utilityTypes || utilityTypes.length === 0) {
      console.warn('[Meters] No utility types found with code water or electric');
      return [];
    }

    const utilityTypeIds = utilityTypes.map((ut: any) => ut.utility_type_id);
    console.log(`[Meters] Found utility types:`, utilityTypes);
    ...
    // Debug logging
    console.log(`[Meters] Fetched ${readings?.length || 0} meter readings`, {
      cycleId,
      roomId,
      hasReadings: readings && readings.length > 0,
...
export default async function MetersPage() {
  ...
  if (!initialReadings || initialReadings.length === 0) {
    try {
      ...
      console.log(`[Meters] Total readings in database: ${totalReadings}`);

      if (totalReadings > 0) {
        // ลอง query แบบง่ายๆ (ไม่ใช้ JOIN หลายตาราง)
        console.log('[Meters] Trying simple query without complex joins...');
        ...
        if (simpleReadings && simpleReadings.length > 0) {
          console.log(`[Meters] Found ${simpleReadings.length} readings with simple query`);
          
          // ตรวจสอบ utility_types
          const utilityTypes = await query<any>('SELECT utility_type_id, code, name_th FROM utility_types');
          console.log(`[Meters] Utility types:`, utilityTypes);
        ...
        console.log('[Meters] No readings found in bill_utility_readings table');
      }
    } catch (error: any) {
      console.error('[Meters] Fallback query failed:', error);
    }
  }

  console.log(`[Meters] Final readings count: ${initialReadings?.length || 0}`);
  if (initialReadings && initialReadings.length > 0) {
    console.log('[Meters] Sample reading:', initialReadings[0]);
  }
